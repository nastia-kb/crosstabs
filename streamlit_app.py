...
with st.expander("Краткое описание"):
            ...
            "- **Число** - вопрос, больше 75% ответов в котором являются числами. По дефолту исключаются выбросы (+/- 1.5 квартильных размаха), в таблицах показывается сумма, среднее, стандартное отклонение и база. При взвешивании числовые переменные также взвешиваются.\n\n" \
            "- **Матрица** - Определяется по наличию 'r_' в коде переменной. Обрабатывается также, как и вопросы выше, но дополнительно выводится сводная таблица общих итогов на отдельном листе\n\n" \
            "- **Тестовый ответ** - вопрос с более чем 150 уникальными вариантами ответа. По дефолту не выводится в таблицах. Если базе есть закрытые вопросы с таким количеством вариантов, итзмените тип вопроса на Один ответ вручную\n\n" \
            "Комментарии к другому (в базе отмечены 'o' в коде вопроса) по дефолту исключаются из анализа. Если в них содержатся важные данные (например, чеки), перед загрузкой файла удалите 'o' из кодов переменных")
